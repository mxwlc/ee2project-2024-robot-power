import asyncio # make sure to download asyncio and websockets
import websockets
...
# used a websocket server to interact with the the web app 
async def websocket_handler(websocket, path):
    ...
    try:
        async for message in websocket:
            ...
            if command[0] == "SET":
                index = bytes([int(command[1])])
                value = float(command[2])
                send(send_q, (SendInstructions.ARDUINO_SET_VAR, index, value))
            elif command[0] == "GET":
                index = bytes([int(command[1])])
                send(send_q, (SendInstructions.ARDUINO_GET_VAR, index))
            elif command[0] == "STOP":
...
async def main():
    async with websockets.serve(websocket_handler,"localhost",9877): # may need changing, port number is arbritary atm
